...
public class HandController : MonoBehaviour {
  ...
  // Frame Delay
  private Queue<Frame> queueFrame = new Queue<Frame>();
  private Queue<Frame> queueFixedFrame = new Queue<Frame>();
  ...
  */
    ...
    if (curr_frame == null || curr_image_frame == null || Time.frameCount != curr_frame_count) {
      if (curr_image_frame != null) {
        ...
        curr_image_frame = null;
			...
			long latency = GetLeapController().Now() - curr_image_frame.Timestamp;
			//UnityEngine.Debug.Log (latency.ToString());
  ...
  protected virtual void Update() {
	...
	Frame newFrame = GetFrame();
		...
		//UnityEngine.Debug.Log ("newFrame: " + (newFrame.Timestamp - GetLeapController().Now()).ToString());

	// Enqueue into the queue
	if (queueFrame == null) {
		queueFrame = new Queue<Frame> ();
	}
	queueFrame.Enqueue (newFrame);
	
	// Dequeue if > queueLength
	if (queueFrame.Count > frameDelay) {
		frame = queueFrame.Dequeue ();
	} else {
		return;
	}
  ...
  protected virtual void FixedUpdate() {
		UnityEngine.Debug.Log ("Time2: " + Time.deltaTime);
	...
	Frame frame;
	Frame newFrame = GetFixedFrame();

	// Enqueue into the queue
	if (queueFixedFrame == null) {
		queueFixedFrame = new Queue<Frame> ();
	}
	queueFixedFrame.Enqueue (newFrame);

	// Dequeue if > queueLength
	if (queueFixedFrame.Count > frameDelay) {
		frame = queueFixedFrame.Dequeue ();
	} else {
		return;
	}
